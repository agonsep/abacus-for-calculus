...
1. **Which unit for the orange stones.** Two options:
   ...
   Decision: **shared unit** for this first version, since the point of the mode is to see `dy` against `y` on one board. If the derivative layer turns out too flat in practice, an own-unit variant can be added later as a separate toggle.
