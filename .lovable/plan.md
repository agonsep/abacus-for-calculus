...
## Goal
When Leibniz Mode is active (including the case where Dual Increments is also checked), reorder the left-panel columns so the mathematical values appear before the stone counts, and use the Greek letter δ in the `delta-y` header.
...
- The header text becomes `δy` (using the Greek lowercase delta character) while keeping the existing orange `palette.change` color.
